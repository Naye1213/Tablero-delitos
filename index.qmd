---
title: "Delitos cometidos en Costa Rica 2023"
author: "Nayely Araya Valerin"
format: 
  dashboard:
    orientation: rows
---

```{r}
#| label: carga_paquetes
#| message: false
#| warning: false

library(tidyverse)
library(plotly)
library(DT)
library(leaflet)
```

```{r}
#| label: carga-datos

delitos <-
  read_delim("estadisticaspoliciales2023.csv")
```

# Generales

## Row

```{r}
#| label: grafico-delitos-nacionalidad
#| title: "Cantidad de delitos por nacionalidad"

grafico_ggplot2 <-
  delitos |>
  group_by(Nacionalidad) |>
  summarize(n = n()) |>
  arrange(desc(n)) |>
  slice(1:7) |>
  ggplot(aes(
    x = reorder(Nacionalidad, -n),
    y = n)) +
  geom_col() +
  xlab("Nacionalidad") +
  ylab("Cantidad")

ggplotly(grafico_ggplot2) |> config(locate = "es")
```

## Row

```{r}
#| label: grafico-tipos-delitos
#| title: "Cantidad de delitos por tipo"

grafico_ggplot2 <-
  delitos |>
  group_by(Delito) |>
  summarize(n = n()) |>
  ggplot(aes(
    x = reorder(Delito, n),
    y = n)) +
  geom_col() +
  coord_flip() +
  xlab("Tipo de delito") +
  ylab("Cantidad")

ggplotly(grafico_ggplot2) |> config(locate = "es")
```

```{r}
#| label: grafico-delitos-provincia
#| title: "Cantidad de delitos por provincia"

grafico_ggplot2 <-
  delitos |>
  group_by(Provincia) |>
  summarize(n = n()) |>
  ggplot(aes(
    x = reorder(Provincia, n),
    y = n)) +
  geom_col() +
  coord_flip() +
  xlab("Provincia") +
  ylab("Cantidad")

ggplotly(grafico_ggplot2) |> config(locate = "es")
```

# Homicidios

```{r}
#| label: filtro-homicidios

homicidios <-
  delitos |>
  filter(Delito == "HOMICIDIO")
```

## Row {.tabset}

```{r}
#| label: grafico-delitos-homicidios
#| title: "Cantidad de homicidios por nacionalidad"

grafico_ggplot2 <-
  homicidios |>
  group_by(Nacionalidad) |>
  summarize(n = n()) |>
  arrange(desc(n)) |>
  slice(1:7) |>
  ggplot(aes(
    x = reorder(Nacionalidad, -n),
    y = n)) +
  geom_col() +
  xlab("Nacionalidad") +
  ylab("Cantidad")

ggplotly(grafico_ggplot2) |> config(locate = "es")
```

```{r}
#| label: grafico-homicidios-provincia
#| title: "Cantidad de homicidios por provincia"

grafico_ggplot2 <-
  homicidios|>
  group_by(Provincia) |>
  summarize(n = n()) |>
  ggplot(aes(
    x = reorder(Provincia, n),
    y = n)) +
  geom_col() +
  coord_flip() +
  xlab("Provincia") +
  ylab("Cantidad")

ggplotly(grafico_ggplot2) |> config(locate = "es")
```

# Otros

## Row

```{r}
#| label: tabla

homicidios |>
  datatable()
```

```{r}
leaflet() |>
  addTiles() |>
  addProviderTiles(
    providers$Esri.WorldImagery,
    group = "Imagenes satelitales (ESRI Wold Imagery)") |>
  setView(
    lng = -84,
    lat = 10,
    zoom = 7
  )
```














